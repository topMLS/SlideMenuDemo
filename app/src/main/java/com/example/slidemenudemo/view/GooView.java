...
import com.example.slidemenudemo.utils.GeometryUtil;
import com.example.slidemenudemo.utils.Utils;
...
public class GooView extends View{
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		dragCenter = new PointF(width,height);//拖拽圆的圆心
		stickyCenter = new PointF(width,height);//固定圆的圆心
		Log.e("gooView",width+"--------------"+height);
	}
	...
	private float dragRadius = 50f;//拖拽圆的半径
	private float stickyRadius = 50f;//固定圆的半径
	private PointF dragCenter;//拖拽圆的圆心
	private PointF stickyCenter;//固定圆的圆心

	private PointF[] stickyPoint = {new PointF(540f, 815f),new PointF(540f, 885f)};
	private PointF[] dragPoint = {new PointF(540f, 815f),new PointF(540f, 885f)};
...
//		path.close();//默认会闭合，所以不用掉
	...
	private float getStickyRadius(){
		...
		radius = GeometryUtil.evaluateValue(fraction, 50f, 5f);
		return radius;
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_UP:
				...
				/*dragRadius=0f;
				stickyRadius=0f;*/
					ViewPropertyAnimator.animate(this).translationXBy(15)
